...
class BLECallbacks : public BLECharacteristicCallbacks
{
  void onWrite(BLECharacteristic *pCharacteristic)
  {
    ...
    if (value.length() > 0)
    {
      ...
      // Add the OTA case here:
      else if (pdoc["type"].as<String>() == "OTA_UPDATE")
      {
        // Convert the received data back to bytes
        std::string firmware_data = pdoc["value"].as<std::string>();
        ...
        if (err != ESP_OK)
        {
          Serial.printf("Error (%s) beginning OTA update\n", esp_err_to_name(err));
        }

        // Write the data to the OTA partition
        err = esp_ota_write(ota_handle, firmware_data.c_str(), firmware_data.length());
        if (err != ESP_OK)
        {
          Serial.printf("Error (%s) writing to OTA partition\n", esp_err_to_name(err));
        }
        err = esp_ota_end(ota_handle);
        if (err != ESP_OK)
        {
          Serial.printf("Error (%s) finalizing OTA update\n", esp_err_to_name(err));
        }

        // Set boot partition to the updated partition and reboot
        err = esp_ota_set_boot_partition(update_partition);
        if (err != ESP_OK)
        {
          Serial.printf("Error (%s) setting new boot partition\n", esp_err_to_name(err));
        }
...
void on_write(esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
  // Get data size and pointer
  size_t len = param->write.len;
  uint8_t *data = param->write.value;

  // If this is the first chunk of a new firmware binary,
  ...
  // Write the received data to the OTA partition
  esp_ota_write(update_handle, (const void *)data, len);
}

// This function is called when a Bluetooth connection is closed
void on_disconnect(esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
  // If we have an ongoing OTA update, finish it
  if (update_handle != 0)
  {
    esp_ota_end(update_handle);

    // Set the OTA partition as bootable and reboot the system
    esp_ota_set_boot_partition(ota_partition);
    esp_restart();
  }
}

// Your Bluetooth event handler
void my_gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
  switch (event)
  {
  case ESP_GATTS_WRITE_EVT:
    on_write(gatts_if, param);
    break;
  case ESP_GATTS_DISCONNECT_EVT:
    on_disconnect(gatts_if, param);
    break;
    // Handle other events...
  }
}
